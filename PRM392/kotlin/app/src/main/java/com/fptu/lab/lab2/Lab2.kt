...
fun random(minRange: Int, maxRange: Int): Int {
    val min = minOf(minRange, maxRange)  // Get the smaller value
    val max = maxOf(minRange, maxRange)  // Get the larger value
...
fun doOperation(first: Double, second: Double, operation: String): Double {
    return when (operation) {
        "+" -> first + second
        "-" -> first - second
        "*" -> first * second
        "/" -> if (second != 0.0) first / second else Double.NaN // Use NaN for division by zero
        else -> 0.0
    }
}
...
fun Lab2_Bai2_Preview() {
    ...
    var result by remember { mutableStateOf(0.0) } // Changed to Double
    var hasGenerated by remember { mutableStateOf(false) }
    var isLoading by remember { mutableStateOf(false) }
    var operation by remember { mutableStateOf("") }

    // Function to handle calculation
    fun performCalculation(op: String) {
        val first = firstNumber.toDoubleOrNull() ?: 0.0
        val second = secondNumber.toDoubleOrNull() ?: 0.0

        isLoading = true
        hasGenerated = false
        operation = op

        // Perform calculation after a delay
        kotlinx.coroutines.GlobalScope.launch {
            kotlinx.coroutines.delay(500) // Shorter delay
            result = doOperation(first, second, op)
            hasGenerated = true
            isLoading = false
        }
    }
    ...
    labTheme {
        Scaffold(
            ...
            Column(
                ...
                // Operation buttons in a row for better layout
                Row(
                    modifier = Modifier.fillMaxWidth(),
                ...
                when {
                    ...
                    hasGenerated -> {
                        Column {
                            Text(text = "Kết quả:", fontSize = 18.sp)
                            ...
                            Column(
                                ...
                                val resultText = when {
                                    result.isNaN() -> "Không thể chia cho 0"
                                    result.isInfinite() -> "Vô cực"
                                ...
                                Text(
                                    text = resultText,
                                    fontSize = 40.sp,
                                    color = if (result.isNaN()) Color.Red else Color.Black,
                                    textAlign = TextAlign.Center
                                )
                            }
                        }
